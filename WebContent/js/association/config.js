...
jQuery(window)
		.load(
				function() {
					...
					jQuery.fn.n33_formerize = function() {
						...
						_form
								...
								.each(
										function() {
											var e = jQuery(this);
											var x = jQuery(jQuery('<div>')
													.append(e.clone()).remove()
													.html().replace(
															/type="password"/i,
															'type="text"')
													.replace(/type=password/i,
															'type=text'));
											if (e.attr('id') != '')
												x
														.attr(
					...
					var _window = jQuery(window), _main = jQuery('#main'), _panels = _main
							.find('.panel'), _body = jQuery('body'), _hbw = jQuery('html,body,window'), _footer = jQuery('#footer'), _wrapper = jQuery('#wrapper'), _nav = jQuery('#nav'), _nav_links = _nav
							.find('a'), _jumplinks = jQuery('.jumplink');

					var panels = [], activePanelId = null, firstPanelId = null, isLocked = false, isTouch = !!('ontouchstart' in window), isLegacyIE = (navigator.userAgent
							.match(/MSIE ([0-9]+)\./) && RegExp.$1 <= 9);
					...
					if (isLegacyIE || skel.isActive('desktop')) {
						...
						_body.h5u_resize = function() {
							var factor = (_window.width() * _window.height())
									/ (1440 * 900);
							_body.css('font-size', Math.min(Math.max(Math
									.floor(factor * settings.sizeFactor),
						...
						_body.h5u_reposition = function() {
							if (isTouch
								...
								_wrapper
										.css(
												'padding-top',
												(((_window.height() - panels[firstPanelId]
														.height()) / 2) - _nav
														.height())
														+ 'px');
						};
						...
						_panels
								.each(function(i) {
									...
									t.h5u_activate = function() {
										...
										// Check lock state and determine
										// whether we're already at the target
										...
										_footer.fadeTo(settings.fadeSpeed,
												0.0001);
